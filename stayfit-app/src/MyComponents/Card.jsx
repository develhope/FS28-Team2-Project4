...
export const Card = ({ title, description, icon }) => {
  ...
  return (
    <button
      onClick={handleClick}
      className={`group flex flex-col p-4 shadow-card active:shadow-card-click transition-all transition-origin-center rounded-xl w-56 h-72 text-[#fff] text-left ${isClicked ? 'scale-110 z-10 bg-activated-card shadow-none' : ''}`}
    >

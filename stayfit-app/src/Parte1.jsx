

const Presentazione = () => {
  return (
    <section className="text-white text-left p-12 py-20 w-full">
      <h2 className="text-5xl font-bold mb-4">IL BENESSERE,
      <br></br>
       PERSONALIZZATO.
       <br></br>
        SEMPLIFICATO
       </h2>
      <p className="text-xl text-white mb-5">Unisciti anche tu alla rivoluzione del Fitness,
      gestisci e monitora i piani di allenamento
      <br></br>
       e nutrizione dei tuoi clienti,
      offrendo soluzioni personalizzate in modo semplice e veloce.
      </p>
      <button className="bg-custom-green text-custom-dark font-bold w-80 border-2 hover:border-hover-orange">
        Unisciti ora!
      </button>
    </section>
  );
};

export default Presentazione;
